...
Every child project has an associated _owner_. Generally speaking, the _owner_ is the user that is in use when making the request. You also have the possibility of declaring a different owner for a child project by specifying an email in the *assigner* parameter and using the translator's/reviewer's session (or email header when using generic authentication). This is the case when the user that will be working on the child project/task makes the POST request and wants to specify a different owner than the parent's project user. Another option to accomplish the same result is to declare the *assignee* parameter using the owner's session (or email header when using generic authentication). This is the case when the individual assignee is known at the moment of the POST request. The *assignee* and *assigner* parameters are optional and mutually exclusive, meaning that you can specify none or one of them (not both). Please note that for the purposes of the DQF hierarchy, an assignee does not have to actually perform the translation/review task. It could also just be a project manager who receives the assignment from their customer. This satisfies the second condition for posting a child project.
