...
The initVector will also be provided by us.
...
POST /v3/project/master/{projectId}/file/{fileId}/targetLang where the target languages are associated with files. The API allows any combination of files/targetLangs. For example file1 has en-US and nl-NL whereas file2 has only en-US as target language.

## Project/Child
This is where the actual work takes place. To declare a child project the 
POST /v3/project/child has to be used. The parent's UUID must be specified to create a parent/child relationship (remember that a tree structure is used). 
Example: 
Child1 will declare the master project's UUID as the parentKey. Of Course child1 can have as many siblings as needed. If then a child of child1 (child1.1) needs to be created , the same endpoint will be used but with the child1 UUID as a parentKey. The type of a child project can be either translation or review. There is also the possibility of directly declaring a different child project owner (other than the one making the request) by optionally specifying an email for the assignee parameter. The email must belong to an existing TAUS account.
